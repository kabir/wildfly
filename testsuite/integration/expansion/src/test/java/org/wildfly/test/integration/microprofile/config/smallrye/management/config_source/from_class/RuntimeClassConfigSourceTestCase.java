...
/**
 * Test for WFLY-21615: Add a class-based config-source at runtime and verify
...
public class RuntimeClassConfigSourceTestCase extends AbstractMicroProfileConfigTestCase {
    ...
    private static final String CONFIG_SOURCE_CLASS = "org.wildfly.test.integration.microprofile.config.smallrye.management.config_source.from_class.CustomConfigSource";
    ...
    public static Archive<?> deploy() {
        return ShrinkWrap.create(WebArchive.class, "RuntimeClassConfigSourceTestCase.war")
                .addClasses(RuntimeTestApplication.class, CustomConfigSource.class)
    ...
    public void testRuntimeConfigSourceAdd() throws Exception {
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            HttpResponse initialResponse = client.execute(new HttpGet(url + "custom-config-source/query?property=" + CustomConfigSource.PROP_NAME));
            ...
            // Step 2: Add config-source with the class at runtime
            addConfigSource();
            ...
            HttpResponse reloadedResponse = client.execute(new HttpGet(url + "custom-config-source/query?property=" + CustomConfigSource.PROP_NAME));
            ...
            Assert.assertTrue(
                    ...
                    "Expected: '" + CustomConfigSource.PROP_NAME + " = " + CustomConfigSource.PROP_VALUE + "' " +
                    "Actual response: " + reloadedText,
                    reloadedText.contains(CustomConfigSource.PROP_NAME + " = " + CustomConfigSource.PROP_VALUE));
        }
    }
    ...
    /**
     * Add a config-source with class via management operation at RUNTIME.
     */
    private void addConfigSource() throws Exception {
        ...
        addOperation.get("class").set(CONFIG_SOURCE_CLASS);
