...
import org.wildfly.extension.microprofile.reactive.messaging.kafka.api.InternalUtil;
...
public class InternalUtilImpl implements InternalUtil {
    ...
    public <K, T> WildFlyKafkaMessage<K, T> from(Message<T> message) {
        if (message instanceof KafkaRecord) {
            return new WildFlyKafkaMessageImpl<>((KafkaRecord<K, T>) message);
        }
        return new WildFlyKafkaMessageImpl<>(KafkaRecord.from(message));
    }
}
