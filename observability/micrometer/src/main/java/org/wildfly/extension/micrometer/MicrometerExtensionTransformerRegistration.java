...
public class MicrometerExtensionTransformerRegistration implements ExtensionTransformerRegistration {
    ...
    private void from2(ResourceTransformationDescriptionBuilder builder) {
        ...
        otlp.addOperationTransformationOverride(WRITE_ATTRIBUTE_OPERATION).setCustomOperationTransformer(OtlpOperationTransformer.INSTANCE);
        otlp.addOperationTransformationOverride(UNDEFINE_ATTRIBUTE_OPERATION).setCustomOperationTransformer(OtlpOperationTransformer.INSTANCE);
        //TODO Maybe some other operations here?
    ...
    private static class OtlpOperationTransformer implements OperationTransformer {
        ...
        public TransformedOperation transformOperation(TransformationContext context, PathAddress address, ModelNode operation) {
            ...
            if (name.equals(ADD) || name.equals(REMOVE)) {
                Long step = operation.hasDefined(STEP.getName()) ? operation.get(STEP.getName()).asLong() : null;
                String endpoint = operation.hasDefined(ENDPOINT.getName()) ? operation.get(ENDPOINT.getName()).asString() : null;
                if (step == null) {
